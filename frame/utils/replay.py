...
from frame.message.can import CANSploitMessage, CANMessage
...
class Replay:
    ...
    def append(self, can_msg):
        """
        可添加两种数据，一种是标准封装好的CAN协议，一种是调试数据，调试数据
        没有时间搓信息。
        """
        if can_msg.CANData:
                ...
                for line in ins:
                    if len(line[:].split(":")) >= 3:
                        ...
                        msg.CANFrame = CANMessage.init_data(
                            num_fid, int(length), bytes.fromhex(data)[:8])
                        msg.CANData = True
                        msg.bus = _bus
            ...
            try:
                ...
                for times, msg in self._stream:
                    ...
                    if not msg.debugData and msg.CANData:
                        _file.write((("[" + str(times) + "]") if times >=
                                    0.0 else "") + msg.CANFrame.get_text() + "\n")
                        curr += 1
                    elif msg.debugData:
