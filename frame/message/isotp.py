...
class ISOTPMessage:
    ...
    def __init__(self, id=0, length=0, data=None, finished=False):
        ...
        :param int id: CANID
        :param int length: CAN消息数据的长度
        :param list data: CAN数据
        :param bool finished: True表明没有更多数据被添加了
        """
        self.message_id = id
    ...
    def _add_frame_single(self, can):
        ...
        :param can0n.can.CANMessage: CAN Message to convert
    ...
    def add_can(self, can):  # Init
        ...
        :param can0n.can.CANMessage can: CAN message
    ...
    def generate_can(fid, data, padding=None):  # generate CAN messages seq
        ...
        :return: List of can0n.can.CANMessage messages in ISO TP format representing the data.
        :rtype: list
        """
        _length = len(data)
        can_msg_list = []
        ...
        if _length < 8:
            ...
            can_msg_list.append(CANMessage.init_data(
                fid, _length + 1 + padding_length, [_length] + data[:_length] + padding_data))  # Single
        elif _length > 4095:
            return []
        else:
            can_msg_list.append(CANMessage.init_data(
                fid, 8, [(_length >> 8) + 0x10] + [_length & 0xFF] + data[:6]))
            seq = 1
            bytes = 6
