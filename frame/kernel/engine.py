...
class L6Engine:
    ...
    def __init__(self, params={}):
        ...
        self._output_screen = True if params.get('output_screen') in [
    ...
    def _get_load_paths(self):
        ...
        # 2. 从用户工作目录开始
        strats.append(os.path.join(
            os.path.expanduser('~'), '.canfuzz', 'modules'))
        # 3. 从包目录
        local_modules_dir = os.path.dirname(
    ...
    def list_modules(self):
        ...
        for search_path in self._get_load_paths()[::-1]:
            ...
            for fullpath in glob.iglob(search_path):
                ...
                if subdir != 'modules':
                    new_module = {os.path.join(subdir, filename): (
                        getattr(module, filename), path)}
                else:
                    new_module = {filename: (
                        getattr(module, filename).name, path)}
        ...
        for action in config.actions:
            ...
            for module, parameters in action.items():
                ...
                self._actions.append(
                    [module, self._modules[module], validated_parameters])
    ...
    def _init_module(self, path, mod, params):
        ...
        # 动态加载模块
        try:
            loaded_module = SourceFileLoader(mod_name, os.path.join(
                search_path, mod_name + '.py')).load_module()
            ...
            for subdir in os.listdir(search_path):
                if os.path.isdir(os.path.join(os.path.abspath(search_path), subdir)):
                    ...
                    try:
                        loaded_module = SourceFileLoader(mod_name, os.path.join(
                            new_search_path, mod_name + '.py')).load_module()
                        self.dprint(1, '加载 {} 从子目录 {}'.format(
                            mod_name, new_search_path))
                        break  # 模块没有找到在子目录
        ...
        #
        self._modules[mod] = getattr(
            loaded_module, mod_name)(params, self.module_ios)
