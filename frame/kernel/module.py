...
class CANModule:
  ...
  def __init__(self, params, ios=None):
    ...
    self.commands['status'] = Command('获取当前状态', 0, '', self.get_status, True)
    self.commands['switch'] = Command('停止/激活 当前模块', 0, '', self.do_activate, True)
    self.commands['outscr'] = Command('停止/激活 标准输出', 0, '', self.do_output_screen, True)
    ...
    self._output_screen = True if params.get('output_screen', False) in ["True", "true", "1"] else False
    self._bus = params.get('bus', self.__class__.__name__)
    self._active = False if params.get('active', False) in ["False", "false", "0", "-1"] else True
    ...
    if in_cmd in self.commands:
      ...
      if cmd.is_enabled:
        try:
          ...
          ret = CmdResult(cmdline=string, describe="执行指令发生异常", last_error=-2, e=e)
          #traceback.print_exc()
      else:
        ret = CmdResult(cmdline=string, describe="命令被禁用", last_error=-2)
    self._thr_block.set()
    return ret
