...
class sniffer(CANModule):
  ...
  def do_init(self, params):
    ...
    self.commands['sniff'] = Command("启用/禁用 嗅探模式", 0, "", self.sniff_mode, True)
    self.commands['print'] = Command("打印已经加载的流量包", 0, "", self.cnt_print, True)
    self.commands['load'] = Command("从文件中加载流量包", 1, "<文件路径>", self.cmd_load, True)
    self.commands['replay'] = Command("从已经加载的流量中回放指定范围的包", 1, "<X>-<Y>", self.replay_mode, True)
    self.commands['save'] = Command("保存指定范围的流量包到文件", 1, "<X>-<Y>", self.save_dump, True)
    self.commands['clean'] = Command("清除已经缓存的包", 0, "", self.clean_table, True)
