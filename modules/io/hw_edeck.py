...
class hw_edeck(CANModule):
  ...
  def do_init(self, params):
    ...
    self.commands['write'] = Command("直接发送CAN数据帧, 类似如下字符串形式: 304:8:07df300101000000", 1, "<数据帧字符串>", self.dev_write, True)
  ...
  def dev_write(self, line):
    ...
    if self._run:
      try:
        ...
        can_msg = CANMessage.init_data(int(idf, 0), int(lenf, 0), dataf)
        message.CANFrame = can_msg
        self.do_write(message)
        ret = CmdResult(cmdline='write ' + line, describe="Edeck设备写入数据", result_type=CMDRES_STR, result=can_msg.get_text())
      except Exception as e:   #cmd 0 t 304:8:07d1300101000000
        self.error('写入CAN数据到设备发生异常', e)
        ret = CmdResult(cmdline='write ' + line, describe="写入CAN数据到设备发生异常", last_error=-2, e=e)
    else:
      ret = CmdResult(cmdline='write ' + line, describe="模块未激活", last_error=-2)
    return ret
        ...
        for device in context.getDeviceList(skip_on_error=True):
          if device.getVendorID() == 0xbbaa and device.getProductID() in [0xddcc, 0xddee]:
            ...
            if self._serial is None or this_serial == self._serial:
              ...
              self.info('打开设备 {0} , {1}'.format(self._serial, hex(device.getProductID())))
              time.sleep(1)
  ...
  def set_can_speed_kbps(self, bus, speed):
    try:
      self._handle.controlWrite(REQUEST_OUT, 0xde, bus, int(speed*10), b'')
    except Exception as e:
