...
class hw_CANSocket(CANModule):
  ...
  def do_init(self, init_params):
    ...
    self.commands['write'] = Command("直接发送CAN数据帧, 类似如下字符串形式: 01A#11223344", 1, " <数据帧字符串> ", self.dev_write, True)
    self._active = True
    self._run = False
  ...
  def dev_write(self, data):
    ...
    if self._run:
      try:
        ...
        ret = CmdResult(cmdline='write ' + data, describe="网络发送数据", result_type=CMDRES_STR, result=can_msg.get_text())
      except Exception as e:
        self.error('写入CAN数据到设备发生异常', e)
        ret = CmdResult(cmdline='write ' + data, describe="写入CAN数据到设备发生异常", last_error=-2, e=e)
    else:
        ret = CmdResult(cmdline='write ' + data, describe="模块没有被激活", last_error=-2)
    return ret
