...
class CustomTCPClient:
  ...
  def handle(self):
    ...
    while self._stop_handle is False:
      try:
        self.socket.sendall(b'c\x01\x00\x00')       # 发送头协议 c\x01\x00\x00
...
class hw_TCP2CAN(CANModule):
  ...
  def do_start(self, params):
    if self._server is None:
      ...
      if self._mode == 'server':
        self._server = CustomTCPServer((self._HOST, self._PORT), ThreadedTCPRequestHandler)
  ...
  def dev_write(self, line):
    ...
    length = line.split(":")[1]
    data = line.split(":")[2]
    can_msg = CANMessage.init_data(int(fid, 0), int(length), bytes.fromhex(data)[:int(length)])
